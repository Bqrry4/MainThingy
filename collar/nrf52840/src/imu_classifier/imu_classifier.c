...
uint64_t stationary_time = 0;
uint64_t motion_time = 0;
static uint64_t last_transition_time;

void countClassTime(uint8_t class)
{

    uint64_t current_time = k_uptime_get();

    uint64_t time_passed = current_time - last_transition_time;
    switch (current_state)
    {
    case 1:
    case 2:
    case 3:
        stationary_time += time_passed;
        break;
    case 4:
        motion_time += time_passed;
        break;
    default:
        break;
    }
                         ...
                         const void *data, uint16_t length)
{
    ...
    countClassTime(((uint8_t*)data)[19]);
...
int imu_classifier_init()
{
    ...
    last_transition_time = k_uptime_get();
